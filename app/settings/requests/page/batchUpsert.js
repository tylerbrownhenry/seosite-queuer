 ...
 */
function Command(baseUrl, originalUrl, linkId, parentLink, requestId, link) {
     return {
          "resolvedUrl": baseUrl,
          "url": originalUrl,
          "_id": linkId,
 ...
 */
function createBulkUpdateCommands(linkObj, requestId, parentLink, foundLinks, baseUrl) {
     var commands = [];
     _.each(foundLinks, function (link) {
          var linkId = sh.unique(link.url.original + requestId);
          commands.push(new Command(baseUrl, link.url.original, linkId, parentLink, requestId, link));
          link._id = linkId;
          linkObj[linkId] = link;
     });
     return commands;
