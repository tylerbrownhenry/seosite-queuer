...
HtmlChecker.prototype.scan = function(html, baseUrl, robots){
	...
	if (this.active === false){
		...
		parseHtml(html).then(function(document){
            ...
            console.log('HtmlChecker.js: Number of links on page',links.length);

            maybeCallback(thisObj.handlers.html)(tree, thisObj.robots,links);
			maybeCallback(thisObj.handlers.links)(links);
			
            _.each(links,function(link){
				enqueueLink(link,thisObj);
            });
...
function excludeLink(link, instance){
    ...
    /* ADDED */
    if(typeof instance.options.checkedLimit !== 'undefined' && instance.options.checkedLimit <= instance.linkEnqueued) return 'BLC_MAX_CHECKED_LIMIT';
	return false;
}
