...
function SiteChecker(options, handlers){
	...
	this.siteUrlQueue = new RequestQueue(
    	...
    	},
    	{
		  item: function(input, done){
            ...
            /*
            We create a base queue, we add the first page to it, and then it is the first one to be scanned.
            We find new links on this page and add them to the queue as well
            */
	...
	this.htmlUrlChecker = new HtmlUrlChecker(this.options,{
		html: function(tree, robots, response, pageUrl, customData){
            console.log('Fetching paging content',response.url,'host?',pageUrl);
			// If was redirected
			if (response.url !== pageUrl){
                pushQueue(thisObj,response.url, true)				
				for (var i=0; i<response.redirects.length; i++){
					// Avoid rechecking any redirected pages
                     pushQueue(thisObj,response.redirects[i].url, true)  
				}
			}
			return maybeCallback(thisObj.handlers.html)(tree, robots, response, pageUrl, customData);
		},
		...
		junk: function(result, customData){
			...
			return maybeEnqueuePage(thisObj, result, customData);
		},
...
SiteChecker.prototype.pause = function(){
    console.log(this.sitePagesChecked);
...
function enqueuePage(instance, url, customData){
    ...
    pushQueue(instance,url,true)
    return instance.htmlUrlChecker.enqueue(url, customData);
}

function checkQueue(instance,url){
    return instance.sitePagesChecked.get(url);
}

function pushQueue(instance,url,value){
    return instance.sitePagesChecked.set(url,value);
}

function maybeEnqueuePage(instance, link, customData){
    var proceed = validatePage(instance, link, customData);
    if(proceed === true){
...
function validatePage(instance, link, customData){
    var attrSupported,
    i,
    redirects,
    tagGroup;
    ...
    if (
        ...
        (link.broken === true) || 
        (link.internal !== true) || 
        (checkQueue(instance,link.url.resolved) === true) || 
        (isAllowed(instance, link) === false)
       ){
        return false;
    }
    
    if (link.url.redirected != null && checkQueue(instance,link.url.redirected) === true){
        ...
        for (i=0; i<redirects.length; i++){
            ...
            pushQueue(instance,redirects[i].url, true)
        }
        return false;
    }
    return true;
